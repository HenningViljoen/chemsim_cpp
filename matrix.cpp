...
matrix::matrix() : m {std::vector<std::vector<double>> ()}//Constructor for populating the matrix by the user himself.
{
    //m = std::vector<std::vector<double>> ();
}

/*matrix::matrix(const matrix *matrixcopyfrom)
{
...
void matrix::solveLYequalsB(matrix &L, matrix &Y, matrix &B) //solves for the X vector the equation: LX = B, with L being a LU decomoposition matrix.
...
void matrix::solveUXequalsY(matrix &U, matrix &X, matrix &Y)
{
      if (U.issquare())
      {
           int Ur = U.m.size();
